...
## Unreleased: BetterMITM next

### BetterMITM-Specific Features

- **Advanced Interceptor**: Complete rewrite with bidirectional interception support (inbound and outbound traffic)
  ...
  - Skip N connections functionality before intercepting
  - Real-time flow state management

- **Enhanced Web Interface (mitmweb)**:
  - Modern, sleek UI design with animations and glint effects
  - 10 beautiful themes (Light, Dark, Blue, Purple, Green, Red, Orange, Cyber, Midnight, Ocean)
  - Advanced flow filtering by type (HTTP, TCP, UDP, DNS, WebSocket, HTTP3, TLS, QUIC) and direction
  - Visual indicators (badges and colors) for paused, blocked, and intercepted flows
  - Improved flow detail editing UI with dropdown for HTTP methods
  - Action buttons (intercept, pause, resume, block, replay, skip) in FlowView
  - Resizable flow detail panel with draggable splitter
  - Fixed overlapping UI elements and improved layout

- **Flow Management**:
  - Flow type filters with multi-select dropdown
  - Direction filter (inbound/outbound/both) with single-select dropdown
  - Skip one connection button
  - Skip N connections button with domain-based configuration
  - Visual state badges in flow path column
  - Improved flow detail panel with better editing capabilities

- **UI/UX Improvements**:
  - Theme selector with popup menu (no longer blocks connection indicator)
  - Discord button linking to community server
  - Improved checkbox styling with proper spacing
  - Fixed FontAwesome icon overlapping issues
  - Enhanced button styling with glint effects
  - Better dark mode text visibility
  - Fixed dropdown text visibility in all themes
  - Improved Advanced Interceptor layout and input fields (now typeable)
  - Fixed splitter resizing behavior

- **Documentation**:
  - Created comprehensive `docs.md` with BetterMITM-specific features
  - Updated `README.md` with BetterMITM branding and features
  - Created `FEATURE_IDEAS.md` with detailed feature proposals
  - Updated `CONTRIBUTING.md` for BetterMITM development

- **Project Structure**:
  - Renamed from `mitmproxy` to `BetterMITM` throughout codebase
  - Updated package name and folder references
  - Updated `run.bat` with automatic `uv` and Node.js installation
  - Improved build process and documentation

### Bug Fixes

- Fixed flow detail panel not opening when clicking on network logs
- Fixed completed connections being pausable (now only live flows can be paused)
- Fixed splitter forcing panel fully to the right when resizing
- Fixed Advanced Interceptor input fields not accepting typed input
- Fixed checkbox and text overlapping in Advanced Interceptor
- Fixed FontAwesome icons overlapping with text in filter input and flow path column
- Fixed intercept URL not appearing in Advanced Interceptor after adding from flow
- Fixed replay functionality (added replay button to FlowView)
- Fixed dropdown text visibility in dark themes
- Fixed checkbox clipping issues
- Fixed connected button and Discord button overlapping
- Fixed keyboard shortcuts interfering with text input in input fields

### Technical Changes

- Extended Flow TypeScript interface with `intercept_state`, `live`, and `killable` properties
- Added API endpoints for Advanced Interceptor rules management
- Added API endpoints for flow state control (pause, resume, block)
- Added API endpoints for skip functionality
- Updated `flow_to_json` to include new flow properties
- Removed all comments from `.ts`, `.tsx`, and Python files (project requirement)
- Fixed TypeScript errors in class components (props, state, setState declarations)
- Updated theme system with CSS variables for all UI components
- Enhanced splitter component with proper resizing logic
