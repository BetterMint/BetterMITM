...
from BetterMITM.utils import emoji
...
class Core:
    ...
    def mark(self, flows: Sequence[flow.Flow], marker: BetterMITM.types.Marker) -> None:
        """
        Mark flows.
        """
        updated = []
    ...
    @command.argument("attr", type=BetterMITM.types.Choice("flow.set.options"))
            ...
            if resp:
                if attr == "status_code":
                    ...
                    if val in status_codes.RESPONSES:
                        resp.reason = status_codes.RESPONSES[val]
                elif attr == "reason":
                    resp.reason = val
                else:
                    supdate = False
    ...
    @command.argument("encoding", type=BetterMITM.types.Choice("flow.encode.options"))
    def encode(
        self,
    ...
    def options_load(self, path: BetterMITM.types.Path) -> None:
        """
    ...
    def options_save(self, path: BetterMITM.types.Path) -> None:
        """
