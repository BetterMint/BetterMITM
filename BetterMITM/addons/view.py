...
from BetterMITM import tcp
from BetterMITM import udp
from BetterMITM.log import ALERT
from BetterMITM.utils import human
...
class _OrderKey:
    ...
    def generate(self, f: BetterMITM.flow.Flow) -> Any:
        pass
...
class OrderRequestStart(_OrderKey):
    def generate(self, f: BetterMITM.flow.Flow) -> float:
        return f.timestamp_created


class OrderRequestMethod(_OrderKey):
    def generate(self, f: BetterMITM.flow.Flow) -> str:
...
class OrderKeySize(_OrderKey):
    def generate(self, f: BetterMITM.flow.Flow) -> int:
        if isinstance(f, http.HTTPFlow):
            size = 0
...
def _signal_with_flow(flow: BetterMITM.flow.Flow) -> None: ...


def _sig_view_remove(flow: BetterMITM.flow.Flow, index: int) -> None: ...


class View(collections.abc.Sequence):
    def __init__(self) -> None:
        super().__init__()
        self._store: collections.OrderedDict[str, BetterMITM.flow.Flow] = (
    ...
    def _bisect(self, f: BetterMITM.flow.Flow) -> int:
        v = self._view.bisect_right(f)
        return self._rev(v - 1) + 1

    def index(
        self, f: BetterMITM.flow.Flow, start: int = 0, stop: int | None = None
    ) -> int:
    ...
    def getvalue(self, flow: BetterMITM.flow.Flow, key: str, default: str) -> str:
        """
    ...
    def setvalue_toggle(self, flows: Sequence[BetterMITM.flow.Flow], key: str) -> None:
        """
    ...
    def setvalue(
        self, flows: Sequence[BetterMITM.flow.Flow], key: str, value: str
    ) -> None:
        """
    ...
    def duplicate(self, flows: Sequence[BetterMITM.flow.Flow]) -> None:
        """
    ...
    def remove(self, flows: Sequence[BetterMITM.flow.Flow]) -> None:
        """
    ...
    def resolve(self, flow_spec: str) -> Sequence[BetterMITM.flow.Flow]:
        """
    ...
    def load_file(self, path: BetterMITM.types.Path) -> None:
        """
    ...
    def add(self, flows: Sequence[BetterMITM.flow.Flow]) -> None:
        """
    ...
    def get_by_id(self, flow_id: str) -> BetterMITM.flow.Flow | None:
        """
    ...
    def update(self, flows: Sequence[BetterMITM.flow.Flow]) -> None:
        """
...
class Focus:
    ...
    def __init__(self, v: View) -> None:
        self.view = v
        self._flow: BetterMITM.flow.Flow | None = None
    ...
    @property
    def flow(self) -> BetterMITM.flow.Flow | None:
        return self._flow

    @flow.setter
    def flow(self, f: BetterMITM.flow.Flow | None):
...
class Settings(collections.abc.Mapping):
    ...
    def __getitem__(self, f: BetterMITM.flow.Flow) -> dict:
