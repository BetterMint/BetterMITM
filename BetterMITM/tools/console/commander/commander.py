...
class CompletionState(NamedTuple):
    completer: Completer
    parsed: Sequence[BetterMITM.command.ParseResult]


class CommandBuffer:
    def __init__(self, master: BetterMITM.master.Master, start: str = "") -> None:
        self.master = master
        self.text = start
    ...
    def render(self):
        ...
        if not parts:
            ...
            for p in parts:
                if p.valid:
                    if p.type == BetterMITM.types.Cmd:
            ...
            if remaining:
                if parts[-1].type != BetterMITM.types.Space:
                    ret.append(("text", " "))
    ...
    def cycle_completion(self, forward: bool = True) -> None:
        if not self.completion:
            ...
            if parts and parts[-1].type != BetterMITM.types.Space:
                ...
                return
            ct = BetterMITM.types.CommandTypes.get(type_to_complete, None)
            if ct:
...
class CommandEdit(urwid.WidgetWrap):
    ...
    def __init__(self, master: BetterMITM.master.Master, text: str) -> None:
