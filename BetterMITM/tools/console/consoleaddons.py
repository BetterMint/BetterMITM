...
from BetterMITM.contentviews import ContentviewMessage
from BetterMITM.exceptions import CommandError
from BetterMITM.log import ALERT
from BetterMITM.tools.console import keymap
...
class ConsoleAddon:
    ...
    def console_choose(
        ...
        cmd: BetterMITM.types.Cmd,
        *args: BetterMITM.types.CmdArgs,
    ) -> None:
        """
    ...
    def console_choose_cmd(
        self,
        prompt: str,
        choicecmd: BetterMITM.types.Cmd,
        subcmd: BetterMITM.types.Cmd,
        *args: BetterMITM.types.CmdArgs,
    ) -> None:
        """
    ...
    def view_pop(self) -> None:
        ...
        user to exit BetterMITM.
        """
        signals.pop_view_state.send()

    @command.command("console.bodyview")
    @command.argument("part", type=BetterMITM.types.Choice("console.bodyview.options"))
    ...
    @command.argument(
        "flow_part", type=BetterMITM.types.Choice("console.edit.focus.options")
    )
    ...
    def grideditor_load(self, path: BetterMITM.types.Path) -> None:
        """
    ...
    def grideditor_load_escaped(self, path: BetterMITM.types.Path) -> None:
        """
    ...
    def grideditor_save(self, path: BetterMITM.types.Path) -> None:
        """
        ...
        try:
            with open(path, "w", newline="", encoding="utf8") as fp:
                ...
                for row in rows:
                    writer.writerow(
                        [strutils.always_str(x) or "" for x in row]
                    )
    ...
    @command.argument(
        "mode", type=BetterMITM.types.Choice("console.flowview.mode.options")
    )
