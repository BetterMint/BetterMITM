...
from BetterMITM.net import check
from BetterMITM.net.check import is_valid_host
from BetterMITM.net.check import is_valid_port
...
def parse(url: str | bytes) -> tuple[bytes, bytes, int, bytes]:
    ...
    parsed_b: urllib.parse.ParseResultBytes = parsed.encode("ascii")
    ...
    full_path: bytes = urllib.parse.urlunparse(
        (b"", b"", parsed_b.path, parsed_b.params, parsed_b.query, parsed_b.fragment)
    )
