...
from BetterMITM.connection import TlsVersion
...
class TlsStartClientHook(StartHook):
    ...
    (by default, this is done by `BetterMITM.addons.tlsconfig`)
    """
    ...
    def receive_handshake_data(
        ...
        try:
            ...
            if last_err in [
                ...
                ("SSL routines", "", "ssl/tls alert bad certificate"),
            ]:
                ...
                in [
                    ...
                    ("SSL routines", "", "record layer failure"),
                ]
                and data[:4].isascii()
            ):
...
class ClientTLSLayer(TLSLayer):
    ...
    def on_handshake_error(self, err: str) -> layer.CommandGenerator[None]:
        ...
        if err.startswith("Cannot parse ClientHello"):
            ...
            in err
            or "('SSL routines', '', 'unsupported protocol')" in err
        ):
            err = (
