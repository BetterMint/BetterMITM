...
import BetterMITM.types
from BetterMITM import command_lexer
from BetterMITM import exceptions
from BetterMITM.command_lexer import unquote
...
def typename(t: type) -> str:
    ...
    to = BetterMITM.types.CommandTypes.get(t, None)
    if not to:
...
class Command:
    ...
    def __init__(self, manager: "CommandManager", name: str, func: Callable) -> None:
        ...
        for name, parameter in self.signature.parameters.items():
            ...
            if not BetterMITM.types.CommandTypes.get(parameter.annotation, None):
                raise exceptions.CommandError(
                    f"Argument {name} has an unknown type {t} in {func}."
                )
        if self.return_type and not BetterMITM.types.CommandTypes.get(
    ...
    def call(self, args: Sequence[str]) -> Any:
        ...
        if ret is None and self.return_type is None:
            return
        typ = BetterMITM.types.CommandTypes.get(self.return_type)
        assert typ
...
class CommandManager:
    ...
    def parse_partial(
        ...
        next_params: list[CommandParameter] = [
            CommandParameter("", BetterMITM.types.Cmd),
            CommandParameter("", BetterMITM.types.CmdArgs),
        ]
        ...
        for part in parts:
            ...
            if expected and expected.kind is inspect.Parameter.VAR_POSITIONAL:
                ...
                expected = CommandParameter("", BetterMITM.types.Unknown)

            arg_is_known_command = (
                expected.type == BetterMITM.types.Cmd and part in self.commands
            )
            arg_is_unknown_command = (
                expected.type == BetterMITM.types.Cmd and part not in self.commands
            )
            command_args_following = (
                next_params and next_params[0].type == BetterMITM.types.CmdArgs
            )
            ...
            to = BetterMITM.types.CommandTypes.get(expected.type, None)
            valid = False
            if to:
                try:
    ...
    def execute(self, cmdstr: str) -> Any:
        ...
        command_name, *args = (
            unquote(part.value) for part in parts if part.type != BetterMITM.types.Space
        )
...
def parsearg(manager: CommandManager, spec: str, argtype: type) -> Any:
    ...
    """
    t = BetterMITM.types.CommandTypes.get(argtype, None)
    if not t:
...
def argument(name, type):
    ...
    specific types such as BetterMITM.types.Choice, which we cannot annotate
