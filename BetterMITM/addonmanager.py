...
class Loader:
    ...
    def add_command(self, path: str, func: Callable) -> None:
        """Add a command to BetterMITM.

        Unless you are generating commands programatically,
        this API should be avoided. Decorate your function with `@BetterMITM.command.command` instead.
        """
    ...
    def register(self, addon):
        ...
        for a in traverse([addon]):
            for old, msg in api_changes.items():
                if hasattr(a, old):
                    logger.warning(
                        f"{msg}. For more details, see https://docs.BetterMITM.org/dev/addons-api-changelog/."
                    )
            name = _get_name(a)
